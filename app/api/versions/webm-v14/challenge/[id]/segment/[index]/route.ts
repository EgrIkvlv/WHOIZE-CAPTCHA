...
import { renderWebmOnlySegment } from "@/apps/captcha-versions/server/webm-only-engine";
import { readWebmOnlySegment } from "@/apps/captcha-versions/server/webm-only-service";
...
export async function GET(
  ...
  try {
    ...
    const result = await readWebmOnlySegment({
      id,
      sessionHash,
      segmentIndex,
    });
    ...
    console.error("Unable to render v1.4 WebM-only segment", error);
    return Response.json(
      { error: "WebM-only video is temporarily unavailable" },
