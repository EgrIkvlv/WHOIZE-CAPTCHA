...
import {
  renderReadableDecoySegment,
  V18B_READABLE_DECOY_COUNT,
} from "@/apps/captcha-versions/server/readable-decoy-engine";
import { readReadableSoloSegment } from "@/apps/captcha-versions/server/readable-decoy-service";
...
export async function GET(
  ...
  try {
    ...
    const result = await readReadableSoloSegment({
      id,
      sessionHash,
      segmentIndex,
    });
    ...
    const segment = await renderReadableDecoySegment({
      ...
      options: { decoyCount: V18B_READABLE_DECOY_COUNT },
    });
    return new Response(Uint8Array.from(segment).buffer, {
      ...
      headers: {
        ...
        "x-whoize-variant": "readable-motion-solo",
        "x-whoize-segment": String(segmentIndex),
      },
    });
  } catch (error) {
    console.error("Unable to render v1.8b readable-solo segment", error);
    return Response.json(
      { error: "Readable-solo video is temporarily unavailable" },
