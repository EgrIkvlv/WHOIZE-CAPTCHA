...
import { readStochasticReadableSegment } from "@/apps/captcha-versions/server/regenerative-motion-service";
...
export async function GET(
  ...
  try {
    ...
    const result = await readStochasticReadableSegment({
      id,
      sessionHash,
      segmentIndex,
    });
    ...
    const segment = await renderRegenerativeMotionSegment({
      ...
      profile: V17_STOCHASTIC_READABLE_PROFILE,
    });
    return new Response(Uint8Array.from(segment).buffer, {
      ...
      headers: {
        ...
        "x-whoize-variant": "stochastic-readable",
        "x-whoize-segment": String(segmentIndex),
      },
    });
  } catch (error) {
    console.error("Unable to render v1.7 stochastic segment", error);
    return Response.json(
      { error: "Stochastic video is temporarily unavailable" },
