...
} from "@/apps/captcha-versions/server/matched-motion-engine";
import { readHumanTunedSegment } from "@/apps/captcha-versions/server/matched-motion-service";
...
export async function GET(
  ...
  try {
    ...
    const result = await readHumanTunedSegment({
      id,
      sessionHash,
      segmentIndex,
    });
    ...
    const segment = await renderMatchedMotionSegment({
      ...
      profile: V15B_HUMAN_TUNED_PROFILE,
    });
    return new Response(Uint8Array.from(segment).buffer, {
      ...
      headers: {
        ...
        "x-whoize-variant": "human-tuned-decoys",
        "x-whoize-segment": String(segmentIndex),
      },
    });
  } catch (error) {
    console.error("Unable to render v1.5b human-tuned segment", error);
    return Response.json(
      { error: "Human-tuned video is temporarily unavailable" },
